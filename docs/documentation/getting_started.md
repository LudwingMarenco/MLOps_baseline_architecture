...
order:
   ...
   b. `data_client_one_monitoring_sensor`: Materializes the monitoring pipeline to evaluate
   serving data quality, feature drift, prediction drift, and model performance. The status is
   computed every 3 minutes.

   c. `data_client_one_serving_sensor`: Triggers the serving pipeline once all ensemble models
   ...
   not met for successful model registration. This sensor perform its validation check every 5 minutes

> **Note:** Retraining in this orchestrator does not involve gathering new data. The purpose is
> to simulate the ML lifecycle, and keeping the dataset updated is a concern that falls on the
